...
\item \code{alpha\_em\_isr} — the $\alpha$ used to build the LL+exp
   ISR exponent $\beta_e = (2\alpha/\pi)(L_e-1)$.\quad\code{None} $\to$
   $\alpha_{G_\mu}(M_W^{\rm BFS\,ref})\approx 1/132.1$, frozen across the
   fit (BFS prescription, line 2514 of \cite{BFSnlo}).
\end{itemize}
...
Setting an explicit float lets the upcoming \code{PARAM\_UNC.alpha\_em}
and \code{PARAM\_UNC.alpha\_em\_isr} nuisances propagate
independently --- the ISR scheme variation decorrelates from the
$\sigma$-chain $\alpha$ uncertainty.
...
\begin{itemize}
\item \code{nll=True} --- the full NLL ePDF (BFS prescription:
  DELTA factorisation, ALGMU renormalisation,
  $\alpha=\alpha_{G_\mu}(\mW^{\rm BFS\,ref})$);
...
production card); \code{isr\_emela\_\{pert\_order,fac\_scheme,ren\_scheme\}}
expose the eMELA scheme so the $\alpha_{\rm em,ISR}$ nuisance can
vary ALGMU$\leftrightarrow$ALPMZ (eMELA returns $\sim 1.6\,\%$ smaller
$\sigma$ at the WW threshold under ALPMZ + $\alpha(M_Z)$ relative to
the BFS default). The two flags are mutually exclusive (runtime
...
eMELA LL (DGLAP), eMELA NLL (DELTA$+$ALGMU) --- we evaluate
...
eMELA NLL (DELTA$+$ALGMU, CodePdf)        & 3.90~ppm & 7.25~ppm \\
\bottomrule
\end{tabular}
\end{table}
...
uncertainty $\sigma_{\mW}\!\approx\!2.30\MeV$ on every cell.
Generated by
...
\item \textbf{NLL ISR.} \textbf{Shipped} (commits \code{2b3f721} and
  ...
  \code{isr\_emela\_ll}, the BFS-prescription default
  (NLL/DELTA/ALGMU/$\alpha_{G_\mu}$) and the ALPMZ scheme variation
  threaded for the $\alpha_{\rm em,ISR}$ nuisance. The cross-fit in
