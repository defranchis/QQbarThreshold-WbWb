...
\item \textbf{$K_C$-safe Coulomb (opt-in).} The production chain
  currently double-counts the leading-$\alpha/v$ Coulomb between $K_C$
  and the first term of BFS eq.~(62) ($\sim 5\%$ on $\sigma$ at
  threshold). An opt-in card knob
  \code{NLO_CONFIG["coulomb_kc_safe"]} retains only the
  $\alpha^2$ subleading Coulomb piece in eq.~(62) when set to
  \texttt{True}; see Appendix~\ref{sec:kcsafe}. Default is
  \texttt{False} (current chain) until an Asimov A/B quantifies the
  $\mW$ shift end-to-end.
\end{enumerate}
...
\clearpage
% ---------------------------------------------------------------
\section{$K_C$-safe Coulomb chain (opt-in)}\label{sec:kcsafe}
% ---------------------------------------------------------------

The Fadin--Khoze--Martin Coulomb resummation $K_C$ (\code{coulomb_K_factor}
in \code{eft_xsec.py}, activated by
\code{NLO_CONFIG["include_coulomb"]=True}) and the BFS NLO Coulomb
correction $\Delta\sigma_\mathrm{Coul}^{(1)}$ of eq.~(62) of
\cite{BFSnlo} (\code{delta_sigma_Coulomb_NLO_specific_pb}, switched on
together with the rest of the NLO loop chain by
\code{NLO_CONFIG["include_NLO_hard_decay"]=True}) describe overlapping
physics on their leading-$\alpha/v$ piece. The first term of eq.~(62),
\begin{equation}
...
\paragraph{The opt-in fix.} A new card knob
\code{NLO_CONFIG["coulomb_kc_safe"]} (default \texttt{False}) selects
between two interpretations of the eq.~(62) addition inside
\code{_add_nlo_loops_to_LR}:
\begin{itemize}
  \item \texttt{False} --- current production chain. The full eq.~(62)
    (term~1 $+$ term~2) is added on top of $K_C$. The
    leading-$\alpha/v$ Coulomb is double-counted at the $\sim 5\%$
    level on $\sigma$ at threshold. This is the chain validated against
    BFS Table~4 with $K_C$ \emph{off} in Scenario~F of
    section~\ref{sec:val-table4}; the $K_C$-on production chain has
    no direct Table-4 cross-check (BFS keeps the Coulomb unresummed
    in their tables).
  \item \texttt{True} --- $K_C$-safe. Eq.~(62) is added with the
    ...
    the subleading Coulomb correction beyond $K_C$.
\end{itemize}
...
\item \textbf{Production-chain magnitude.} The $K_C$-safe combination
  reduces $\sigma_\mathrm{partonic}$ (default chain: PDG-constant BR,
  full NLO$+$NNLO, $\delta_\mathrm{QCD}$, Whizard anchor) by $-7.9\%$
  at $\sqrts=157\GeV$, $-7.7\%$ at $161\GeV$, $-6.5\%$ at $163\GeV$.
  The shift is smooth in $\sqrts$ with no new kinks;
  $\mathrm{d}\sigma/\mathrm{d}\mW$ and $\mathrm{d}\sigma/\mathrm{d}\GammaW$
  retain the same overall shape. Diagnostic at
  \code{plots/coulomb_double_count/compare_kc_safe_chain.pdf}.
\end{itemize}

\paragraph{Expected fit impact.} A naive single-energy estimate
$\Delta\mW\simeq -\Delta\sigma/(\partial\sigma/\partial\mW)$ gives
$\Delta\mW\sim\mathcal{O}(100)\MeV$ at each individual $\sqrts$; this
over-counts the actual shift because a near-constant
$\Delta\sigma/\sigma$ across the scan is absorbed by the luminosity
nuisance. The residual $\sim 0.3\%$ \emph{shape} slope across
$[157,163]\GeV$ is the part that survives the fit and is what drives
the central $\mW$ shift --- consistent with the several-MeV
prediction of the deep-audit memo of 2026-05-20. A clean A/B
(\code{coulomb_kc_safe} $\in\{\mathrm{False},\mathrm{True}\}$) on the
Asimov fit at the FCC-ee operating scenario is the next step before
flipping the default; until then, the $K_C$-safe combination is left
opt-in so the current chain remains bit-exact and the parallel
investigations on the calculation are not perturbed.
...
depend on $K_C$, NLO loops, or the \code{coulomb_kc_safe} setting. The
morph is therefore the correct anchor for both the current and the
$K_C$-safe chains, and no morph regeneration is needed when the knob
is flipped.
