...
the strongest $\mW$ sensitivity — is item~(iv), the decay
substitution, which is implementable in our framework as a
\code{br\_convention}-like card option. See
...
\begin{enumerate}
\item \textbf{BFS recipe details on the NLO addition.} A detailed
  closure-budget campaign (section~\ref{sec:val-table4-budget}) shows
  that the residual to BFS Table~4 at $\sqrts\in[161,170]\GeV$ is
  dominated, not by NLL ISR effects, but by two
  BFS-recipe details we do not yet implement:
  \begin{itemize}
  \item The $\hat\sigma_{LR}^{(0)}\to\hat\sigma_{\rm Born}$
    substitution in the decay correction (BFS line 2660--2661 of the
    paper source): adds $\sim 0.5\,\%$ to $\sigma_{\rm obs}$ at
    $\sqrts=161\GeV$, $\sim 0.2\,\%$ at $\sqrts=170\GeV$. Implementable
    as a card knob in
    \code{delta\_sigma\_NLO\_decay\_specific\_pb}; trivial code change,
    closes the scan-window residual to MC-stat.
  \item The $\sqrt{x_1 x_2 s}>155\GeV$ cut on the LL$+$exp
    convolution of $\Delta\hat\sigma_{\rm NLO}$ (BFS line
    2664--2666): $\lesssim 0.1\,\%$ at $\sqrts=161\GeV$, slightly larger
    at $\sqrts=158\GeV$. Trivial to add.
  \end{itemize}
  After these two fixes the closure budget is dominated by (a) the
