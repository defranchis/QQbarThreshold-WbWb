...
\item \texttt{grid} — trilinear interpolation in $(\sqrts,\mW,\GammaW)$
  ...
  A 2331-point highstats variant
  ($9\,\mW \times 7\,\GammaW \times 37\,\sqrts$ at $\sim$0.016\,\% MC stat,
  i.e.\ 25-MeV $\mW$ step) plus a threshold-region densification
  (extra 12 $\sqrts$ points at 0.25-GeV step in $[157.25, 162.75]$~GeV)
  is in flight; the morphing scheme it feeds is described in
  section~\ref{sec:morphing}.
\end{itemize}
...
\subsubsection{Morphing scheme (highstats grid)}\label{sec:morphing}

For the highstats grid the operational predictor is not a trilinear
interpolation of $\sigma_\mathrm{Whizard}$ directly but a per-$\sqrts$
analytic morph fitted from the grid:
\begin{equation}
...
The bilinear cross-term coefficient peaks at $\beta\simeq+0.31$~GeV$^{-2}$
...
quadratic morphing (without $\beta$) leaves a $\sim$0.16\,\% residual at
the worst $\sqrts$; the cross term reduces it to MC-bar level
($\sim$0.08\,\%). Leave-one-out cross-validation on the $\sqrts$ axis
confirms the cubic spline along $\sqrts$ adds no precision overhead:
the median LOO residual at interior $\sqrts$ sits at the per-point MC
bar (0.013\,\%), the maximum below $0.1\,\%$ at the peak-rise.
The construction is taken apart and validated extensively in
...
$(\sqrts,\mW,\GammaW)$ cube: 2331 highstats points
($37\times9\times7$) plus 756 densify points at half-step $\sqrts$,
each at $\sim$0.016\,\% Monte-Carlo precision
(Fig.~\ref{fig:mv-grid}).
...
\item \textbf{Closure.} The predictor is checked against held-out
WHIZARD points at 1-MeV $(\mW,\GammaW)$ steps
(Fig.~\ref{fig:mv-substep}) and against the published BFS WHIZARD 4f
...
spline threads clean points.

\subsection{Input grid}\label{sec:morphval-grid}

The morph is built from a single rectangular WHIZARD grid. The $(\mW,\GammaW)$ block is identical at every $\sqrts$ --- this is what makes the factorised per-$\sqrts$ fit possible --- and the Monte-Carlo precision is uniform at the few-$10^{-4}$ level across the scan.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\linewidth]{morph_grid_overview}
\caption{The WHIZARD 3.1.5 highstats grid the morph is built on. Left:
...
$(\mW^0,\GammaW^0)$ for the highstats ($0.5\GeV$ $\sqrts$ spacing) and
densify ($0.25\GeV$) campaigns --- flat at $\sim$0.016\,\%, the floor
against which all morph residuals below are compared.}
\label{fig:mv-grid}
\end{figure}
...
with slope $\beta$; (middle) the fitted $\beta(\sqrts)$, peaking near
$+0.3\GeV^{-2}$ at $\sqrts\simeq160.5\GeV$ and decaying to zero away
from threshold; (right) the maximum residual over the doubly-off-axis
points, naive versus full morph, versus $\sqrts$ --- the cross term
brings it down to the MC bar everywhere.}
\label{fig:mv-cross}
\end{figure}

\subsection{$\sqrts$ interpolation}\label{sec:morphval-sqrts}

Between grid $\sqrts$ the eight morph quantities are carried by natural cubic splines through the denoised grid. Fig.~\ref{fig:mv-sqrts-abs} shows the quantities themselves and Fig.~\ref{fig:mv-sqrts} the deviation of the raw per-$\sqrts$ fit from the denoised curve actually used; Fig.~\ref{fig:mv-sqrtsval} is a blind test on the half-step densify points, and Fig.~\ref{fig:mv-4dloo} the leave-one-out cross-check.
...
denoised curve the operational morph carries. $\sigma_0$ is the steep
threshold line shape; $\beta$ is the bilinear cross term, peaking near
$+0.3\GeV^{-2}$ at the rise; $R_m$ and $R_\Gamma$ are the $m_W$ /
$\GammaW$ morph factors, shown at $\mp100\MeV$ and $\mp40\MeV$ off
nominal. Fig.~\ref{fig:mv-sqrts} shows the same quantities as the
deviation from the denoised curve.}
...
propagated if the grid were not denoised. $\beta$ is the noisiest
quantity ($\sim$20\,\% per node). \code{denoise_grid} removes the
ripple by $\chi^2$-smoothing the slowly-varying ratios $\sigma/\sigma_0$
along $\sqrts$ and averaging $\sigma_0$ across the $(\mW,\GammaW)$
plane, so the morph carries the smooth zero-line curve.}
\label{fig:mv-sqrts}
\end{figure}
...
\caption{Blind test of the $\sqrts$ interpolation. The densify points
lie at the $0.25\GeV$ midpoints between the highstats $\sqrts$ nodes;
predicting them from a morph built on the highstats grid \emph{only}
tests the cubic spline at genuine un-fitted $\sqrts$ (a stronger test
than leave-one-out, which removes a node the spline would otherwise
anchor on). Left: the per-point residual vs $\sqrts$ (top) and its
median and maximum (bottom) --- MC-bar limited, maximum $0.08\,\%$.
Right: the pull $(\sigma_\text{morph}-\sigma_\text{WHIZARD})/
\delta\sigma_\text{MC}$, centred on zero (no systematic bias); its
width $\sim$1.4 --- down from $\sim$1.6 before the grid was denoised
--- shows the morph prediction is now interpolation-limited rather
than dominated by propagated node noise.}
...
limited; the spikes at the two extreme $\sqrts$ are LOO extrapolation
artifacts --- the operational interpolator never leaves the grid. The
denoised blind test of Fig.~\ref{fig:mv-sqrtsval} is the leak-free
companion measure.}
\label{fig:mv-4dloo}
\end{figure}
...
Two independent closure tests bound the total morph error: held-out WHIZARD points at 1-MeV $(\mW,\GammaW)$ spacing (Fig.~\ref{fig:mv-substep}) and the published BFS WHIZARD 4f Born tables (Fig.~\ref{fig:mv-bfs}).
...
scatters within the MC band with no trend --- maximum $0.04\,\%$,
median $0.01$--$0.02\,\%$ --- so the morph is sub-MeV-safe at the
...
Table~2 ($\mW=80.379$, $\GammaW=2.09201\GeV$) reference points and
...
$<0.15\,\%$ at every $\sqrts$; the morph/BFS ratio tracks the
raw-grid/BFS ratio closely, so the residual is the genuine
...
$\GammaW$-induced crossing at $\sqrts\simeq161.9\GeV$, consistent with
