...
$\sigma_{\mW}\!=\!1.22\MeV$, $\sigma_{\GammaW}\!=\!1.53\MeV$. Versus
...
$\sigma_{\GammaW}$ moved from $1.05\MeV$ to $1.53\MeV$, consistent
...
residual gap on $\mW$ is expected to close once \code{isr\_nll=True}
becomes the production default (the eMELA-NLL ISR run is gated on
the parallel ISR-default flip).
...
\item \textbf{NLL ISR.} \textbf{Shipped} (commits \code{2b3f721} and
  ...
  statistical target. Production card defaults remain
  \code{isr\_nll=False} (so the LL$+$exp baseline is preserved for
  paper-closure tests); switching to \code{isr\_nll=True} is the
  recommended production setting at precision targets and requires no
  recipe changes elsewhere in the chain.
