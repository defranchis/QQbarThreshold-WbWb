...
constexpr double sqrt_s_min = 230.0;
constexpr double sqrt_s_max = 300.0;
...
struct sqrt_s_loop {
    static void compute_xsec(QQt::pert_order::order order, float mass, float width, 
        ...
        if (Index < N){
            ...
            const double xsec = QQt::ttbar_xsection(
                    sqrt_s, {mass_scale, width_scale}, {mass, width}, order,
                    opt
                 );
...
PYBIND11_MODULE(xsec_single_top, m) {
