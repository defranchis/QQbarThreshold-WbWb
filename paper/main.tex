...
integrated luminosity of $12\abinv$ split equally among the scan points.
\TODO{the integrated luminosity and its distribution among scan points
are placeholder values pending the finalisation of the FCC-ee \WW
operation scenario; the results scale accordingly.}
...
to FCC-ee projections~\cite{dEnterria:2020cpv}. The beam-related
uncertainties are split into a point-to-point uncorrelated and a
correlated component, with the size of the correlated component assumed
to be half of the uncorrelated one. We assume a luminosity uncertainty of
$0.1\%$ (uncorrelated) and $0.05\%$ (correlated) per scan point, a beam
energy calibration uncertainty of $10$ ($5$)\MeV for the uncorrelated
(correlated) component, and a beam energy spread uncertainty of $1\%$
($0.5\%$) for the uncorrelated (correlated) component.
\TODO{the BEC and BES input uncertainties are placeholder values; their
dependence is studied explicitly in Section~\ref{sec:param_scans}, and
the impact of additional systematic sources will be assessed once the
detector-level study is available.}
...
Experimental (stat.)              & 1.1 & 2.3 & $\mathcal{L} = 12\abinv$ (placeholder) \\
Parametric \mW                    & --  & 2.5 & free parameter \\
Parametric \GW                    & 1.5 & --  & free parameter \\
Parametric \as                    & 0.1 & 0.0 & $\delta\asmz = 10^{-4}$ \\ \hline
Luminosity calibration (uncorr.)  & 1.4 & 2.3 & $\delta L/L = 0.1\%$ \\
Luminosity calibration (corr.)    & 1.1 & 0.0 & $\delta L/L = 0.05\%$ \\
Beam energy calibration (uncorr.) & 0.7 & 1.1 & $\delta\sqrts = 10\MeV$ \\
Beam energy calibration (corr.)   & 0.6 & 0.0 & $\delta\sqrts = 5\MeV$ \\
Beam energy spread (uncorr.)      & 0.0 & 0.0 & $\delta\Delta E = 1\%$ \\
Beam energy spread (corr.)        & 0.0 & 0.0 & $\delta\Delta E = 0.5\%$ \\
\hline
Total experimental                & 2.5 & 4.0 & \\
Theory (ISR, unprofiled)          & 5.4 & -- & NLL ISR (Section~\ref{sec:theory_unc}) \\
\end{tabular}
     ...
     described in Section~\ref{sec:theory_unc}. The luminosity and its
     distribution among scan points, as well as the BEC and BES input
     uncertainties, are placeholder values, and additional experimental
     and machine-related systematic uncertainties are not yet included.}
     \label{tab:breakdown}
\end{table}
...
with a total experimental precision of $2.5$ and $4.0\MeV$,
respectively, taking into account the experimental, parametric, and
machine-related uncertainties considered so far. The statistical
component alone amounts to $1.1$ and $2.3\MeV$ for \mW and \GW. The
leading systematic contribution to \mW is the uncertainty in the
integrated luminosity, while the precision in \GW is driven by the
statistical uncertainty and by the parametric correlation with \mW. A
correlation of $61\%$ between \mW and \GW is obtained from the fit, as
shown by the two-dimensional confidence intervals in
Figure~\ref{fig:contour}. We stress that the absolute size of the
luminosity-related uncertainties and the absence of detector-level and
additional machine-related systematic uncertainties imply that the
...
We perform a cross-fit in which the pseudo-data are generated with one
ISR scheme and the templates are built with another, and read off the
resulting bias in the fitted \mW. Using LL ISR with the templates, while
the pseudo-data are generated at NLL, biases \mW by about $22\MeV$, far
above the FCC-ee statistical target. This bias decomposes into a
component of about $17\MeV$ associated with the truncation of the LL
series, which is removed by adopting a fully exponentiated NLL
description, and a residual component of about $5.4\MeV$ associated with
the genuine NLL terms. We adopt the NLL prediction as the baseline and
take the residual $5.4\MeV$ as a conservative estimate of the residual
ISR theoretical uncertainty on \mW, as reported in
Table~\ref{tab:breakdown}. In addition, the uncertainty in the value of
$\alpha(\mZ)$ that enters the ISR, propagated as a nuisance parameter
according to the FCC-ee projection, is found to shift \mW by less than
$0.05\MeV$ and is therefore negligible. We also verified that the
...
\GW as free parameters, we project a statistical precision of about $1.1$
and $2.3\MeV$ on \mW and \GW, respectively, and a total experimental
precision of $2.5$ and $4.0\MeV$ including the leading parametric and
machine-related uncertainties considered so far. We studied the
...
uncertainty associated with the modelling of ISR to be about $5.4\MeV$ on
\mW.
...
be included, and the assumed integrated luminosity and its distribution
among scan points are placeholder values pending the finalisation of the
FCC-ee operation scenario. Nonetheless, the present study demonstrates
...
\begin{equation}
    ...
    C_\mathrm{lumi}^\mathrm{corr},
\end{equation}
where $C_\mathrm{exp}$ is the diagonal covariance matrix of the measured
cross sections, with
$[C_\mathrm{exp}]_{ij} = \delta_{ij}\,\sigma_\mathrm{exp}^i/\mathcal{L}_i$,
and $C_\mathrm{lumi}^\mathrm{uncorr}$ and $C_\mathrm{lumi}^\mathrm{corr}$
