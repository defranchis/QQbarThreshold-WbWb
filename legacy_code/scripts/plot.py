...
def get_Xsec(order,scheme, mass='', yukawa='', width = '', alphaS='Nominal'):
    ...
    df = pd.read_csv('output/{}_scan_{}_noISR_mass{}_width{}_yukawa{}_as{}.txt'.format(order,scheme,mass,width,yukawa,alphaS), sep=',')
    df.columns = ['ecm', 'xsec']
    #print(df)
    return df
...
def doPlotScheme(scheme):
    ...
    plt.savefig('plots/plot_{}.png'.format(scheme))
    plt.clf()
    ...
    plt.savefig('plots/plot_{}_ratio.png'.format(scheme))
    plt.clf()
    return
...
def compareSchemes(order):
    ...
    plt.ylabel('Cross Section')
    plt.title('Cross Section vs Energy')
    plt.savefig('plots/plot_{}.png'.format(order))
    plt.clf()
...
def compareMasses(order,scheme):
    ...
    plt.ylabel('Cross Section')
    plt.title('Cross Section vs Energy')
    plt.savefig('plots/plot_{}_{}_mass.png'.format(order,scheme))
    plt.clf()
...
def compareYukawa(order,scheme):
    ...
    plt.ylabel('Cross Section')
    plt.title('Cross Section vs Energy')
    plt.savefig('plots/plot_{}_{}_yukawa.png'.format(order, scheme))
    plt.clf()
    ...
    plt.savefig('plots/plot_{}_{}_yukawa_ratio.png'.format(order, scheme))
    plt.clf()
...
def compareAlphaS(order,scheme):
    ...
    plt.ylabel('Cross Section')
    plt.title('Cross Section vs Energy')
    plt.savefig('plots/plot_{}_{}_alphaS.png'.format(order, scheme))
    plt.clf()
    ...
    plt.ylabel('Ratio of Cross Section')
    plt.title('Ratio of Cross Section vs Energy')
    plt.savefig('plots/plot_{}_{}_alphaS_ratio.png'.format(order, scheme))
    plt.clf()
...
def compareWidthYukawaAlphaS(order,scheme):
    ...
    addTitles(plt,order)
    plt.savefig('plots/plot_{}_{}_width.png'.format(order, scheme))
    plt.clf()
    ...
    plt.savefig('plots/plot_{}_{}_yukawa_alphaS_ratio.png'.format(order, scheme))
    plt.clf()
...
#
    #for order in orders:
    #    compareSchemes(order)
#
    #for order in orders:
    #    compareMasses(order,'PS')

    #compareYukawa('N3LO','PS')
    #compareAlphaS('N3LO','PS')
